...
export const DEMO_TOOLS = [
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "font": {
                    "type": "string",
                    "description": "Optional: typeface to load, e.g. 'arialbd.ttf' or a full path. A bare filename is resolved against the OS font directory. Blender's built-in font is a thin sans with no bold variant, so a real bold legend needs an actual bold .ttf here."
                },
                "offset": {
                    "type": "number",
                    "description": "Optional: fattens the glyph outline in place (faux-bold), in scene units. Use a small positive value like 0.1-0.3 at size 10; too much closes up the counters of letters such as A, B and O."
                },
                "rotation": {
                    "type": "array",
                    "items": {
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "size": {
                    "type": "number",
                    "description": "cube/plane edge size"
                },
                "radius": {
                    "type": "number",
                ...
                "subdivisions": {
                    "type": "integer",
                    "description": "icosphere subdivisions"
                }
            },
            "required": [
                "type",
    ...
    },
    {
        ...
        "description": "Build a WATERTIGHT extruded plate from a 2D outline with through-holes and engraved regions in one mesh, no booleans. Use INSTEAD of create_polygon + boolean_operation for printable flat parts with holes/text (boolean cutouts often go non-manifold and slicers silently delete them). Returns is_watertight; success is false if not watertight.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "name": {
    ...
    {
        "name": "create_curve",
        "description": "Create a curve for precise 2D/3D drafting: POLY/BEZIER/NURBS/PATH splines, or a mixed line + TRUE-ARC profile via 'segments' (exact arcs from center/radius/angles — use this for radius-gauge-style geometry, never tessellated points). Supports grid snapping, extrusion into a solid, bevel, and fill. Result stays an editable CURVE; run convert_to_mesh before booleans or export.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "name": {
                ...
                "spline_type": {
                    ...
                    "default": "BEZIER",
                    "description": "Spline type for 'points' mode. POLY = straight segments (linear drafting). PATH = NURBS clamped to its endpoints. Ignored in 'segments' mode (always exact-arc Bezier)."
                },
                "points": {
                    "type": "array",
                    "items": {
                    ...
                    "description": "[[x,y],...] or [[x,y,z],...] control points for a single spline. Provide either 'points' OR 'segments', not both."
                },
                "segments": {
                    "type": "array",
                    "items": {
                        ...
                        "properties": {
                            ...
                            "end_deg": {
                                "type": "number",
                                "description": "For 'arc': end angle in degrees. end > start sweeps counter-clockwise; end < start sweeps clockwise."
                            }
                        },
                        "required": [
                            "type"
                    ...
                    "description": "Mixed profile of line and TRUE-ARC segments, joined into one continuous Bezier spline (exact arc handles, split into <=90deg pieces). Segments whose endpoints coincide are welded."
                },
                "cyclic": {
                    "type": "boolean",
                    "default": false,
                ...
                "extrude": {
                    "type": "number",
                    "default": 0,
                    "description": "TOTAL solid height in scene units (mm). The filled curve becomes a solid spanning z = location.z .. location.z + extrude."
                },
                "bevel_depth": {
                    "type": "number",
                    "default": 0,
                ...
                "snap": {
                    "type": "number",
                    "description": "Grid-snap increment (e.g. 0.5): every input coordinate is quantized to the nearest multiple before building the curve."
                },
                "location": {
                    "type": "array",
                    "items": {
    ...
    {
        "name": "extract_sketch",
        "description": "Reverse of create_curve: dump sketched CURVE and grease-pencil geometry as world-space JSON (control points, cyclic flags, Bezier handles, stroke points). Read-only. Feed the result to scripts/sketch_to_session.py to fit lines/arcs into a replayable session.json.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "pattern": {
    ...
    {
        "name": "apply_modifier",
        "description": "ADDS and configures a modifier - despite the name it does NOT bake it. ALWAYS follow with apply_all_modifiers, or the modifier stays live and the next operation sees unmodified geometry. Verify with the vertex count, not the success flag. POWER TIP: Use 'target_objects' to add AND sync this modifier to multiple objects in ONE call! This is much faster than adding modifiers one-by-one or using copy_modifier.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "object_name": {
                ...
                "modifier_type": {
                    "type": "string",
                    "description": "ARRAY, SOLIDIFY, BEVEL, MIRROR, SUBSURF, WIREFRAME, SMOOTH, BOOLEAN, DECIMATE, SCREW, SIMPLE_DEFORM, TRIANGULATE, REMESH. Only these types are configured; any other is created but left at its defaults while still reporting success."
                },
                "name": {
                    "type": "string",
                ...
                "octree_depth": {
                    "type": "integer",
                    "description": "For REMESH in BLOCKS/SMOOTH/SHARP mode: resolution of the octree scan. Higher is finer and far heavier."
                },
                "voxel_size": {
                    "type": "number",
                ...
                "quad_method": {
                    "type": "string",
                    "description": "For TRIANGULATE: BEAUTY (default), FIXED, FIXED_ALTERNATE or SHORTEST_DIAGONAL"
                },
                "ngon_method": {
                    "type": "string",
                ...
                "axis": {
                    ...
                    "description": "For SCREW (lathe/spin/revolve): axis to revolve the profile around. The profile (a 2D curve profile converted to mesh, or an edge chain) must sit offset from this axis in the plane perpendicular to it, e.g. an XZ profile with axis='Z' traces a cup/vase wall from the object's local origin outward."
                },
                "angle_deg": {
                    "type": "number",
                    "default": 360,
                ...
                "deform_method": {
                    ...
                    "default": "TWIST",
                    "description": "For SIMPLE_DEFORM: deformation type. TWIST rotates geometry progressively along deform_axis (e.g. a spiral-fluted vase on top of a SCREW-lathed ribbed profile). BEND curves it, TAPER/STRETCH scale it along the axis."
                },
                "deform_axis": {
                    "type": "string",
                    "enum": [
    ...
    },
    {
        ...
        "description": "Boolean operation between objects or collections. SLICE cuts a hole AND keeps the piece as a new object; operand_type='COLLECTION' uses all objects in a collection as cutters. CRITICAL: object_a must NOT be inside collection object_b (self-subtraction deletes it). Prefer the 'EXACT' solver. The cutter is auto-hidden.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "object_a": {
    ...
    },
    {
        ...
        "description": "Split one mesh's disconnected shells into separate objects. A boolean that cuts a part into pieces leaves ONE object holding several shells, and delete_object works per object - so use this when the pieces must be handled individually. Returns the new object names, sorted.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "object_name": {
                ...
                "prefix": {
                    "type": "string",
                    "description": "Optional. Rename the results to <prefix>_0, <prefix>_1, ... in sorted order, instead of keeping Blender's auto-generated .001/.002 names."
                }
            },
            "required": [
                "object_name"
    ...
    },
    {
        ...
        "description": "PRIMARY TOOL for a 3D architectural shell from a 2D floor plan: call ONCE for the whole building outer perimeter (ordered [x,y] vertices), never per room. Creates {name}_Floor, {name}_Walls, {name}_Ceiling. Then add interior partitions with build_wall_segment / build_wall_with_door.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "vertices": {
    ...
    },
    {
        ...
        "description": "Plain interior partition wall (no door): a surface from floor Z=0 to ceiling height. Workflow: build_room_shell first, then this for solid partitions, build_wall_with_door for partitions with openings.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "start_point": {
    ...
    },
    {
        ...
        "description": "Interior wall with a door opening (open aperture, no booleans). Door is centred by default; pass door_offset to place it off-centre.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "start_point": {
    ...
    {
        "name": "generate_views",
        "description": "Render orthographic top/front/side/iso previews of the scene in one call. Temporary cameras and lights are framed automatically from the scene's bounding box and removed afterwards, so no camera/light/render commands are needed in the session.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "views": {
    ...
    {
        "name": "sculpt_grab",
        "description": "Grab-brush style sculpt: move vertices near a 3D location by an offset vector. Uses smooth cosine falloff: vertices at the center move the full offset, vertices at the radius edge are barely moved. Use this to pull a boat bow into a point, push in dents, raise terrain hills, etc.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "object_name": {
    ...
    {
        "name": "check_design",
        "description": "Look up 3D-printing design rules bearing on a part you are about to model, described in plain language (e.g. 'a snap-fit enclosure lid with 2mm walls and pin joints'). Returns wall minimums, clearances, overhang limits and similar constraints, each cited to the video that stated it. Consult this BEFORE generating geometry, not after -- these constraints are cheap to honour while modelling and expensive to discover after a failed print. A null value means the source stated no number, so choose a dimension knowingly rather than treating the rule as inapplicable.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string",
                    "description": "What you are modelling: features, joints, materials, dimensions."
                },
                "limit": {
                    "type": "integer",
    ...
    },
    {
        ...
        "description": "Filter the 3D-printing design rules by keyword, topic or unit. Prefer check_design when you can describe the part in prose. Consult this BEFORE generating geometry, not after -- these constraints are cheap to honour while modelling and expensive to discover after a failed print. A null value means the source stated no number, so choose a dimension knowingly rather than treating the rule as inapplicable.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "keyword": {
                    "type": "string",
                    "description": "Substring to match in the rule text, e.g. 'overhang', 'wall', 'clearance'."
                },
                "topic": {
                    "type": "string",
                    "enum": [
                ...
                "measured_only": {
                    "type": "boolean",
                    "description": "Only rules whose number was actually stated in the video, excluding conventional defaults.",
                    "default": false
                },
                "limit": {
                    "type": "integer",
    ...
    },
    {
        ...
        "description": "List the design-rule topics with how many rules each holds and how many carry a stated dimension. Useful for judging whether the knowledge base can answer a question before asking it.",
        "inputSchema": {
            "type": "object",
            "properties": {}
        }
